...
public class MemberDaoJdbcImpl implements MemberDao {
	...
	public void insert(Member member) {
		PreparedStatementCreatorFactory psCreatorFactory = new PreparedStatementCreatorFactory(
				"insert into member(MemID, LastName, FirstName,MiddleName,Status,Password) values(?,?,?,?,?,?)",
				new int[] { Types.CHAR, Types.CHAR, Types.CHAR});
		KeyHolder keyHolder = new GeneratedKeyHolder();
		int count = jdbcTemplate.update(
				psCreatorFactory.newPreparedStatementCreator(new Object[] {
						member.getMemid(), member.getLastname(),
		...
		// member.setMemid(keyHolder.getKey().toString());
	}

	public void update(Member member) {
		int count = jdbcTemplate
				.update("LastName, FirstName,MiddleName,Status,Password) = (?,?,?,?,?) where id = ?",
						member.getLastname(), member.getFirstname(),
						member.getMiddlename(), member.getStatus(),
						member.getPassword());
		if (count != 1)
	...
	public void delete(String memid) {
		int count = jdbcTemplate.update("delete member where id = ?",
				memid);
		if (count != 1)
...
//				"select MemID, FirstName, LastName from tblMembers where MemID = ?",
...
//	}
	...
	public List<Member> findByStatus(String status) {
		PreparedStatementCreatorFactory psCreatorFactory = new PreparedStatementCreatorFactory(
				"select * from tblMembers where status = ?",
				new int[] { Types.CHAR });
		return jdbcTemplate.query(psCreatorFactory
				.newPreparedStatementCreator(new Object[] { status }),
				new RowMapper<Member>() {
					public Member mapRow(ResultSet rs, int rowNum)
							throws SQLException {
						Member member = new Member();
